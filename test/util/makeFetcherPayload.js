...
test("Returns a JSON payload request when request have no variables", t => {
  const expected = {
  ...
  const actual = makePayload(paramsTemplate)

  t.deepEqual(expected, actual)
  t.true(isJSONValid(actual.body))
})

test("Returns a JSON payload when body does not contain files", t => {
  const params = {
    ...paramsTemplate,
  ...
  const actual = makePayload(params)

  t.true(isJSONValid(actual.body))
})

test("Returns form-data payload whrn body have at least one file", t => {
